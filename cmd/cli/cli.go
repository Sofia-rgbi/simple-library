...
func hadlerChoise(userChoise int, myLibrary *library.Library, scanner *bufio.Scanner) {
	...
	case 1:
		...
		scanner.Scan()
		author := scanner.Text()
		_, err = myLibrary.AddBook(title, author, year)
		if err != nil {
			fmt.Println(err)
		}
	//Добавить читателя
	case 2:
		...
		err := myLibrary.AddReader(firstName, lastName)
		if err != nil {
			fmt.Println(err)
		}
	//Удалить книгу
...
func Run(myLibrary *library.Library) {
	...
	for {
		...
		if userChoise == 15 {
			if err := storage.SaveBooksToCSV("books.csv", myLibrary.Books); err != nil {
				fmt.Printf("Произошла ошибка сохранения книг: %v", err)
			}
			if err := storage.SaveReadersToCSV("readers.csv", myLibrary.Readers); err != nil {
				fmt.Printf("Произошла ошибка сохранения пользователей: %v", err)
			}
			break
		}
	}
